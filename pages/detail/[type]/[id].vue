        ...
        <div class="mt-auto" v-if="!data.isbuy">
          <template v-if="type == 'book'">
            <template v-if="menus.length > 0">
              ...
              <n-button v-if="freeID" @click="learn({ id: freeID })" strong secondary type="info" class="ml-2"> 免费试看 </n-button>
            </template>
      ...
      <n-grid-item :span="18">
        ...
        <section class="detail-bottom">
          <UiTab class="border-b">
            <UiTabItem :active="tabState == item.value" v-for="(item, index) in tabs" :key="index" @click="changeTab(item.value)"
              >{{ item.label }}
            </UiTabItem>
          </UiTab>
          <!-- 课程是图文类型并且已经购买则显示课程内容，否则显示介绍 -->
          <div v-if="tabState == 'content'" class="content" v-html="data.type == 'media' && data.isbuy ? data.content : data.try"></div>
          <!-- 详情目录 -->
          <DetailMenu v-else>
            <DetailMenuItem v-for="(item, index) in menus" :key="index" :index="index" :item="item" @click="learn(item)" />
...
const { tabs, tabState, changeTab } = useInitDeatailTabs(type);
