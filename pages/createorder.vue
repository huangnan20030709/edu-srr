...
<template>
  <LoadingGroup :pending="pending" :error="error">
    <n-card class="createorder" size="medium">
      <h4>产品信息</h4>
      <div class="flex mb-5">
        <n-image :src="data.cover" class="flex flex-shrink-0 rounded" :class="type == 'book' ? 'w-[100px] h-[140px]' : 'w-[180px] h-[100px]'" />
      ...
      <n-grid :x-gap="20" :cols="4" class="mb-3">
        <n-grid-item v-for="(item, index) in couponData.rows" :key="index">
          <n-button strong secondary :type="user_coupon_id == item.id ? 'success' : 'tertiary'" @click="chooseCoupon(item)">
            ￥{{ item.price }} 优惠券
          </n-button>
        </n-grid-item>
      </n-grid>

      <div v-if="couponData.count == 0" class="mb-5 text-gray-400 text-sm">暂无优惠券</div>
      <h4>支付方式</h4>
      <div class="flex mb-5">
        <span class="wxpay">
          <n-icon>
